...
class BYOL(torch.nn.modules.loss._Loss):  #不用负样本，本文的参考对象
    """
...
# class OurLoss(torch.nn.modules.loss._Loss):  #ContraWR
#
...
class OurLoss(torch.nn.modules.loss._Loss):  # ContraWR
    ...
    def forward(self, emb_anchor, emb_positive):
        ...
        # compute instance-aware world representation, Nx1
        sim = torch.mm(emb_anchor, emb_positive.t()) #很重要
        weight = self.softmax(sim)
        # print(weight)
        neg = torch.mm(weight, emb_positive)
        #
...
class OurLoss_PRO(torch.nn.modules.loss._Loss):  # ContraWR

    def __init__(self, device, margin=0.5, sigma=2.0, T=2.0):
        """
        T: softmax temperature (default: 0.07)
        """
        super(OurLoss_PRO, self).__init__()
        self.T = T
        self.device = device
        self.margin = margin
        self.softmax = torch.nn.Softmax(dim=1)
    ...
    def forward(self, emb_anchor, emb_positive_1, emb_positive_2):
        # L2 normalize, Nxk, Nxk
        emb_anchor = F.normalize(emb_anchor, p=2, dim=1)
        emb_positive_1 = F.normalize(emb_positive_1.float(), p=2, dim=1)
        emb_positive_2 = F.normalize(emb_positive_2.float(), p=2, dim=1)

        # compute instance-aware world representation, Nx1
        sim_1 = torch.mm(emb_anchor, emb_positive_1.t()) / self.T
        weight_1 = self.softmax(sim_1)
        neg_1 = torch.mm(weight_1, emb_positive_1)

        # compute instance-aware world representation, Nx1
        sim_2 = torch.mm(emb_anchor, emb_positive_2.t()) / self.T
        weight_2 = self.softmax(sim_2)
        neg_2 = torch.mm(weight_2, emb_positive_2)

        # representation similarity of pos/neg pairs
        l_pos_1 = torch.exp(-torch.sum(torch.pow(emb_anchor - emb_positive_1, 2), dim=1) / (2 * self.sigma ** 2))
        l_neg_1 = torch.exp(-torch.sum(torch.pow(emb_anchor - neg_1, 2), dim=1) / (2 * self.sigma ** 2))

        # representation similarity of pos/neg pairs
        l_pos_2 = torch.exp(-torch.sum(torch.pow(emb_anchor - emb_positive_2, 2), dim=1) / (2 * self.sigma ** 2))
        l_neg_2 = torch.exp(-torch.sum(torch.pow(emb_anchor - neg_2, 2), dim=1) / (2 * self.sigma ** 2))

        zero_matrix = torch.zeros(l_pos_1.shape).to(self.device)
        loss = torch.max(zero_matrix, l_neg_1 - l_pos_1 + l_neg_2 - l_pos_2 + self.margin).mean()
        # print('loss',loss)
        return loss

# class OurLoss_pseudo(torch.nn.modules.loss._Loss):  # ContraWR
#
#     def __init__(self, device, margin=0.5, sigma=2.0, T=2.0):
#         """
#         T: softmax temperature (default: 0.07)
